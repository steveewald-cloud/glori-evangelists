"""
GLORi Evangelists transactional email module.

Uses the Resend API (https://resend.com) if RESEND_API_KEY is set as a Fly secret.
If the key is not configured, sends are skipped and logged as warnings rather than
raising, so the app never 500s because of a missing email provider.
"""
...
def _send(to_email: str, subject: str, html: str) -> bool:
    if not RESEND_API_KEY:
        logger.warning(
            "RESEND_API_KEY not set - skipping email to %s (subject: %s)", to_email, subject
        )
        return False
    try:
        resp = httpx.post(
...
def send_invite_email(to_email: str, name: str, token: str) -> bool:
    ...
    your territory, clients, commissions, and attainment in real time.</p>
...
def send_dispute_filed_email(admin_email: str, rep_name: str, dispute_type: str, description: str, dispute_id: int) -> bool:
...
def send_dispute_resolved_email(to_email: str, rep_name: str, status: str, resolution_notes: str) -> bool:
    ...
    <p style="text-align:center; margin:32px 0;">
      <a href="{link}" style="background:{GOLD}; color:{NAVY}; text-decoration:none; font-weight:bold;
      padding:14px 28px; border-radius:6px; display:inline-block;">View My Dashboard</a>
    </p>
    """
